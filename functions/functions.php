...
function cart(){
	if(isset($_GET['add_cart'])){
		...
		if(mysqli_num_rows($run_check)>0){
			...
			$insert_pro = "insert into cart values ('$pro_id','$ip', 1)";
		...
		if(!isset($_GET['brand'])){
			...
			while ($row_pro=mysqli_fetch_array($run_pro)) {
				...
				echo " <div id='single_product'>
					...
					<a href='index.php?add_cart=$pro_id'><button style='float:right; font-weight:bold'>Add to Cart</button></a>
					</div>";
			}
		}
	}
...
function getCatPro(){
	if(isset($_GET['cat'])){
		...
		if ($count_cats==0) {
			echo "<h3 style='padding:20px;'>No products were found in this category!</h3>";
		}
		else{
...
function getBrandPro(){
	if(isset($_GET['brand'])){
		...
		if ($count_brands==0) {
			echo "<h3 style='padding:20px;'>No products were found for this brand!</h3>";
		}
		else{
